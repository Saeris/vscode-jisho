...
The prose is hand-written; the **claims it makes** are asserted. See [docs/specs/19-documentation-drift-tests.md](../../../docs/specs/19-documentation-drift-tests.md).

- `src/__tests__/readme.spec.ts` checks the README against `package.json`: every setting documented with its **real default**, every command either documented or declared deliberately omitted, every image referenced present as a light/dark pair. Runs in the default suite, needs no browser.
- `e2e/docs/claims.docs.e2e.ts` asserts behavioural claims against a running VS Code — that romaji resolves, that conjugated input finds the dictionary form, that hovers work in Markdown and **do not** in code files. Runs with `vp run docs:shots`, or on its own with `vp run docs:claims`.

**When you add a factual claim to the README, add its assertion.** A claim nobody asserts is one nobody will notice going stale — that is exactly how "select the reading to hear it spoken" survived review while describing a control that was not there.

**When one of these fails, the fix is usually to edit the prose.** The test failing means the product changed and the manual now lies. Update both in the same commit.

Some claims cannot be tested (download sizes, "no telemetry", roadmap intent). Spec 19 lists them, so the suite is not mistaken for total coverage.
...
- If you changed a setting, a command, or any behaviour the README describes, run `vp run docs:shots` (which includes the claims) and fix what fails.
