...
**As-built:** **Separate download confirmed by measurement** — `jisho-names.db` is **409MB** (130MB gzipped) for 743,456 entries, _larger_ than the full word DB, so bundling was never viable. Delivered as its own `jisho-names.db.gz` trio on `dictionary-latest` (`vp run build:data:names`). Per the user, **default-on now** (provisioned lazily on the first names query, cached), to become **opt-in via the BACKLOG #14 preferences menu** later — nothing is published yet, so no migration is needed. Structure: `src/data/names-schema.sql` (name_words/kanji/kana/translations/search_terms/tags), simpler than words (translation replaces senses/pos/glosses). Host: `NamesDictionary` (`src/host/names.ts`) with `searchNames`/`getName`, a **separate connection** from `Dictionary` (the word DB never waits on names). Delivery: `ensureNamesDatabase` (download-only, no bundled dev copy required) + a `prefix` param on `downloadDatabase`. Extension: a parallel lazy `#names` promise + `respondNames` dispatch; `searchNames`/`getName` are **separate messages** (not folded into `search`), so the names query in the webview runs independently — a first-use names-DB download never blocks word/kanji results. UI: a third "Names" results section + a simplified `NameDetail` view (type badges) + a `nameDetail` navigation view. `name` ranking is coarse (JMnedict has no commonness flag; many 田中 entries), acceptable for a secondary feature — recorded in the names tests. `assets/**` is already `.vscodeignore`d, so `jisho-names.db` never ships in the `.vsix`.
