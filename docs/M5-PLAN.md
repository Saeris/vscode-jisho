...
- **#11 suggestion strip deferred, and its verdict was corrected mid-M5.** The web platform genuinely cannot _override_ the OS IME candidate window (non-cancelable composition `beforeinput`; `chrome.input.ime` is ChromeOS-only). F5 testing then showed our search field gets no typeahead prediction window while a native Windows text field does — researched to a Chromium-uses-IMM32-not-TSF limitation (both windows are OS-drawn; Chromium's IMM32 path forgoes Windows "text intelligence" typeahead), _not_ any app widget. Regardless, an **app-rendered dictionary suggestion strip is viable** — it's our own DOM, owes nothing to the OS IME, and coexists with it. #11 stays deferred pending a cross-OS key-binding spike (mac/Linux IMEs), not because it's infeasible — see the corrected verdict in [BACKLOG.md](BACKLOG.md) #11. #12 (arrow-nav) shipped.
