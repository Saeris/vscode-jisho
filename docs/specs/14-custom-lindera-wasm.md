...
**Upstream drafts prepared (in scratchpad, awaiting user review — NOT posted):** `upstream-issue.md` (bug report, reproduction, root cause, two fix options) and `upstream-pr.md` (a `release.yml` one-step patch adding `napi build` to the publish job, with an honest verification section noting maintainers must confirm the packed tarball in CI). Both to be reviewed by the user before anything is submitted to `lindera/lindera` under their identity.
