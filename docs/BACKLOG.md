...
### 12. Arrow-key navigation between search box and results (fix — medium) — ✅ shipped

All three parts are implemented in `SearchResults.tsx` and, as of 2026-07-30, actually covered: ↓ from the input into the first result, ↑/Esc back out, and the guard that makes ↑ leave the list ONLY from the first option (intercepting it unconditionally would break moving up within the list). The tests were impossible while component specs ran under jsdom — focusing a React Aria ListBox option needs layout APIs it lacks — so they were deferred to E2E and then never written there, with a component test asserting the searchbox merely EXISTS standing in. Moving the component project to a real browser made them expressible; each is verified by breaking the handler and confirming the right test fails.
...
- **Rendered by a component**, for settings that change what is rendered rather than how it looks — `appearance.tagLabels` (#50), `appearance.colorExamples` (#38). Same snapshot, but read through `useHostSettings` (a `useSyncExternalStore` over the bridge) because CSS can neither swap 名詞 for "noun" nor omit an attribute.

**Note the deliberate split between `highlighting.enabled` and `appearance.colorExamples`** (decided 2026-08-02). Both colour Japanese by part of speech, but they gate different surfaces and default differently, and collapsing them was considered and rejected. `highlighting.enabled` decorates the user's OWN Markdown and plain-text files — a change to documents they are editing, so opt-in, default **off**. `colorExamples` covers example sentences inside the panel, which is the extension's own surface where the breakdown bar and tag pills are already coloured unconditionally — so default **on**. One toggle would have forced either uncoloured examples on a default install (inconsistent with every other panel surface) or decorations turned on in the user's files without asking. Each setting's description cross-links the other via `#setting.id#`.
...
### 16. Breakdown bar: filter the sentence in place instead of destructive re-search (fix — medium) — ✅ shipped 2026-08-06

The M5 segment bar makes each content word a tappable chip, but tapping one **replaces** the search input with that lemma and re-searches — the original sentence is lost, and there's no way back to the previous fragment (it only survives word-detail back-navigation, not chip-to-chip). jisho.org's model is better: the breakdown is a **filter over the current sentence**, matching one segment at a time while the full sentence stays in the input, so you can move between fragments. Rework the chip action to select-a-segment (highlight the active fragment, drive the results filter) rather than overwrite the query. Deferred from M5 as polish.

**As built (2026-08-06).** Two premises in the note above were wrong. The machine did **not** reserve a selected-segment index — `NavContext` had only `stack`, `forwardStack` and `searchQuery`, so the field had to be added. And the filter needs no new query: `search` already passes every content lemma to the host as a deinflection candidate, so one response contains entries for all of them and selecting a chip narrows what is already on screen — the same client-side approach the tag filter (#27) uses on the same list, and why switching chips is instant.

Matching is on the primary headword/reading only; matching any writing pulls in entries that merely share a form (measured on 私, which also returns 妾 and 儂). The selection is an INDEX so a repeated word (行って…行く) stays separately selectable, lives in machine context so Back returns to the filtered view, and is cleared by every action that changes the query. `ToggleButtonGroup` in single-selection mode owns the selection — which renders the chips as `radio`/`radiogroup`, not `button`/`aria-pressed`.
...
- **POS semantic highlighting** — PROTOTYPE SHIPPED (2026-07-18): a DocumentSemanticTokensProvider on markdown/plaintext colors Japanese by part of speech at the MORPHEME level (auxiliaries distinct from their verb stem, so conjugation structure is visible), ruby-markup aware (a {食|た}べる group colors whole as its word). POS maps onto built-in token types so every theme colors it: verb→function, noun→variable, adjective→type, adverb→property, particle→keyword, auxiliary→macro. Off by default (`vscode-jisho.highlighting.enabled`) — recoloring documents is opinionated; the toggle applies live via onDidChangeSemanticTokens. Pure-kana runs stay uncolored (same tokenizer constraint as the hover — wrong boundaries would teach wrong boundaries). Awaiting the user's visual refinement pass: the type mapping is the first knob to turn; custom token types + semanticTokenScopes fallbacks are the upgrade path if the built-ins prove too arbitrary.
  ...
  **Round 2 direction — custom palettes + color-vision accessibility (user design input, 2026-07-18).** The user authored a POS palette for their Japanese slide decks (11 categories: pronoun, noun, adnominal, adjectival noun, adjective, particle, adverb, conjunction, interjection, verb, auxiliary verb; light + dark variants; designed toward CVD friendliness) and flagged that theme-hooking cannot serve color-vision-deficient users — different palettes per CVD type may be needed. Decisions:
  ...
  - ~~**BLOCKED on the palette's actual values**~~ — **UNBLOCKED and shipped.** The user supplied the Figma palette, and it was re-derived in OKLCH with three natively-built dichromacy variants (`appearance.palette`); see docs/pos-palette-research.md for the derivation and citations, docs/palette-tools/ for the generators.
  - **Surfaces, as shipped (2026-08-02): four, not two.** Editor decorations (`highlighting.enabled`), SegmentBar breakdown chips, grammar tag pills (#50), and example sentences (`appearance.colorExamples`). All four resolve `data-pos` through ONE mapping — `src/webview/styles/posCategory.css` for the three webview surfaces, `posDecorations.ts` for the editor — so a category can never mean different colours on different surfaces. The mapping was duplicated per-component until the example sentences would have made it a third copy.
  - **Example sentences: 4 of 9 categories → all 9** — ✅ shipped 2026-08-02 (user report). Example colouring rode the build-time `[word](pos:id)` links, and `LINKABLE_POS` in `build-data.ts` emits links for content words only (`n`/`v`/`adj`/`adv`). Measured before the fix: **68.7% of characters sat inside a link** and could be coloured; the other **31.3% could not** — particles, auxiliaries, pronouns, adnominals and utterances rendered as plain foreground. That is why examples looked unlike the editor's highlighting of the same sentence, and it left the uncoloured grammatical connectives as the brightest text on the line — the emphasis exactly inverted.

    **As shipped:** the markup gained a second form, `[は](p:)` — a part of speech with an EMPTY id, meaning "colour this, don't link it" (`posToken` in `shared/exampleLinks.ts`). `LINKABLE_POS` stays as the LINK policy (opening a JMdict entry for は helps nobody, and making 29%-of-tokens tappable would bury the useful links); it simply stopped doubling as the COLOUR policy. No new tokenization: the build already called `segment()` on every sentence and had all nine categories — it was discarding five.

    One grammar covers both forms (`(\d*)`, not `(\d+)`) so `exampleText` strips them in a single pass. A second token syntax was rejected for exactly this reason: it would be a second thing every consumer must know about, which is how `[もっと](adv:1012620)` once leaked into the word page.

    **Measured after the rebuild:** POS coverage **68.7% → 93.4%** of characters (the remaining 6.6% is punctuation and `other`, which correctly carry no claim). 653,812 link tokens + 680,683 span tokens. `ja_furigana` grew 24.31 → **27.16 MB raw** (predicted 27.48), and the shipped zstd-19 artifact is **24.09 MB**.

    **Rejected: tokenizing in the webview.** Measured both costs rather than assuming. The spans cost **+13% raw but only +1.1% compressed (~+0.05 MB)** — `](p:)` repeated hundreds of thousands of times is nearly free to zstd, so raw size is the wrong metric for a compressed artifact. Against that, the tokenizer is a **4.9 MB native Node addon** that cannot load in a webview at all; it would need a WASM build of Lindera plus its IPADIC dictionary in the bundle. It would also re-derive, from surface text alone, matches the build makes against JMdict with lemma+reading — so the two passes could disagree, colouring a word one way while it links another.

    **Pronouns and adnominals are linked too** (author request, 2026-08-02) — they are real dictionary words a reader may want to look up, and both are small CLOSED classes (79 and 47 distinct surfaces across the corpus), so their resolution is verifiable rather than open-ended. Linking them safely needed two guards, because the resolver's old take-the-first rule was wrong in exactly the cases a reader would notice:
    - **`POS_CONFIRM`** — a candidate entry must actually carry `pn`/`adj-pn`. Without it 彼 resolved to the あれ "that" entry and 君 to the honorific suffix "Mr". No agreement ⇒ no link (still coloured), because for a closed class a missing link is far cheaper than a misleading one.
    - **`INVARIANT_POS`** — the linked surface must EQUAL a dictionary form. Pronouns and adnominals never inflect, so a longer segment means the tokenizer merged in trailing material: そのこと → その, この時 → この, あの方たち → あの. Measured at **8.9% of adnominal and 5.7% of pronoun links** before the check. This must never be generalised to verbs or adjectives — 食べました legitimately links to 食べる, and that longer span is the entire point of the word-boundary work.

    Result: pronouns link at **93.6%**, adnominals at **87.4%**, with **zero** trailing defects; the refused remainder degrades to a coloured span rather than a wrong link. Particles, auxiliaries and utterances stay at 0% linked by design.

    **The accuracy is guarded, because this data is DERIVED** (the author's requirement — Tatoeba ships neither the POS tags nor the links). `db.spec.ts` → "example annotation accuracy" runs against the built DB on every build and asserts the properties rather than specific ids: every pronoun link targets a `pn` entry, every adnominal link an `adj-pn` one, closed-class surfaces equal a dictionary form, particles/auxiliaries are never linked, and ≥90% of characters carry a POS. A tokenizer upgrade or JMdict release that degraded any of these would fail the suite instead of shipping quietly.

    **Revisit if an isomorphic WASM Lindera lands** (author, 2026-08-02). A runtime pass is the more flexible design — it would drop the stored markup entirely and let colouring follow tokenizer improvements without a data rebuild. It is blocked on upstream: there is no WASM build to depend on today, and writing one is not this project's job. The build-time markup is the right answer _now_, not a permanent verdict; the seam to change is `annotateExample` (emit) plus `parseExampleMarkup` (consume), both already isolated in `shared/exampleLinks.ts`.
...
The real constraint is size, not capability: ~400 MB in OPFS is untenable (per-origin quotas, eviction, and a hostile first-run on vscode.dev). So the web build ships a **different data tier** — the common subset by default (**now 101 MB raw / 39 MB gzipped, not the 51 MB / 21 MB this was scoped against** — the Tatoeba pool doubled it, so the tier needs re-deciding), full DB as an explicit opt-in, names DB not offered (409 MB, measured). **Also note the tokenizer is no longer WASM** — see the stale-premise note in spec 06. Known upstream risk, non-blocking: turso documents an OPFS **write**-path hang (mid-transaction cache spilling); our browser workload is read-only, and seeding should write the file to OPFS directly rather than INSERTing rows.
...
**Diagnosis + primary fix DONE (2026-07-25).** Probed the code: the Lindera tokenizer is already CORRECT on the hard cases (いいよ→いい+よ, 勉強する vs 勉強, して's し→lemma する). The false positives were DOWNSTREAM — the hover discarded the tokenizer's POS and searched the lemma STRING, so ranking picked the wrong homophone (search「する」→擦る "to rub"; search「し」→死). Fixed with `Dictionary.resolveByLemma(lemma, pos)` (src/host/db.ts) + `src/shared/pos.ts` (coarse↔JMdict-code compatibility): the hover now resolves via the tokenizer's (lemma, POS) directly, POS-filtered, with a `uk`/kana-primary tiebreak that floats 為る above 擦る (freq_rank is backwards for usually-kana words). Verified in a live hover E2E (する→"to do", not "to rub"/"death"). Remaining under this item: the eval corpus + scorer, and the deinflect() rewrite (§a below).

**§a — deinflect() over-generation (Yomitan-style typed transforms).** The rule-based `deinflect()` (src/host/deinflect.ts) is now only the FALLBACK for pure-kana/romaji queries the tokenizer can't segment, but it over-generates with no POS constraint (して→[する,しる], たかくない→[たかくる,たかくい,たかい]). Rewrite with Yomitan's typed-transform model — each rule carries `conditionsIn`/`conditionsOut` over a POS-condition hierarchy (v→v1/v5…, `isDictionaryForm` flags), and a deinflection chain is valid only if conditions chain up AND the entry's POS is compatible. Study yomidevs/yomitan `ext/js/language/ja/japanese-transforms.js` (GPL — technique, not code) + jconj.
...
### 49. E2E restructure: per-feature suites, light+dark in one run, less UI noise (infrastructure — medium) — ✅ done 2026-07-30
...
- ~~**Theme switch mid-run, not per-launch.**~~ **Done.** `Launched.setTheme(kind)` rewrites the profile's `settings.json` and waits for `.monaco-workbench` to report the new theme kind. The picker was the racy part, not the switch: it fought quick-input for focus (which lives inside our webview), whereas a file write has a settled end state to wait for — and VS Code watches that file. `visual-light.e2e.ts` is gone; its four captures are a `test.describe("light theme")` block at the end of the serial `visual.e2e.ts`, so the audit costs four captures instead of a second 8-second boot.
- ~~**Kill the visual noise.**~~ **Done, but the premise above was wrong** and the correction is the useful part. The deliberate captures were never contaminated: `screenshotSidebar` shoots `.part.sidebar`, and both the Chat panel (right side) and the toast (bottom-right) fall outside it. The noise was only ever in Playwright's full-window **failure** screenshots — still worth fixing, since those are the artifact `e2e.yml` uploads, but a debugging surface rather than the review loop. Verified by capturing a full window before and after rather than by reading setting names. What worked: `extensions.ignoreRecommendations` + `workbench.enableExperiments: false` seeded, and `closeChatPanel()` in `launchVSCode`. What did **not**: `chat.commandCenter.enabled`, `workbench.secondarySideBar.showTabs`, `workbench.panel.showLabels` — all three left the panel open, because side-bar visibility lives in the profile's `state.vscdb` workbench state, **not in `settings.json`**, and so cannot be seeded at all. That is why this one step is a UI action, and why it awaits its own effect (`waitFor({ state: "hidden" })`) instead of clicking and hoping.
- ~~**Shared helpers**: a `capture(name)` that shoots both themes, and a `cleanChrome()`~~ **Dropped as unnecessary.** `cleanChrome()` has no callers left — `closeChatPanel` runs once inside `launchVSCode`, so every spec gets clean chrome without opting in. A both-themes `capture()` would switch themes per capture; batching one switch for the whole light block is the same coverage for one-tenth the switching.
- ~~**Per-feature suites: deferred, and the file split is the wrong lever.**~~ **Done, and the deferral reasoning was wrong.** I argued that `workers: 1` makes each new file pay its own `beforeAll(launchVSCode)`, so splitting six ways trades one boot for six. True of a per-file `beforeAll`; not true of a WORKER-scoped fixture, which launches once for the whole run no matter how many files ask for it. `e2e/fixtures.ts` now provides that, `e2e/visual.e2e.ts` is six files under `e2e/visual/`, and the suite went from 1.5m to ~33s because sharing removed three of the four launches rather than adding five.

  Two things the split forced into the open. `settings.e2e.ts` genuinely cannot share — it seeds `vscode-jisho.*` config that must exist before activation — which is exactly the "different launch settings" exception the deferral predicted, and it keeps its own instance. And sharing means workbench state now survives across FILES: the first capture started failing the moment it stopped getting a fresh instance. Hence the test-scoped `jisho` fixture, which hands back the webview reset to an empty search, so no file depends on which files ran before it. `theme.e2e.ts` restores the dark theme in `afterAll` for the same reason — Playwright makes no promise about file order, and alphabetically it runs BEFORE `word-detail`.
...
**Reference:** [Jisho.org's search-operator docs](https://jisho.org/docs) for tag vocabulary and naming.

**Target taxonomy (author-supplied during design, 2026-08-02).** Seven top-level categories, each marked by whether our shipped data can derive it:

| Category                | Subcategories                                                                                                                                                                                                                                                  | Derivable from what we ship?                                                                                                                                                                                                                                |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Common words**        | Frequency bands of 2000 (1–2000, 2001–4000, … to ~22,600)                                                                                                                                                                                                      | ✅ `words.freq_rank` (15,147 populated). The reference list's total is essentially our 22,624 common set — the same JMdict slice — so the banding transfers directly.                                                                                       |
| **Words by JLPT level** | N5 … N1                                                                                                                                                                                                                                                        | ✅ `words.jlpt` (7,208 matched). Carries the same caveat we already show: approximate only, since official lists have not been released since 2010.                                                                                                         |
| **Language parts**      | Adjectives, Adverbs, Auxiliary, Conjunctions, Counters, Interjections, Nouns, Onomatopoeic, Particles, Prefixes, Suffixes, Verbs, Yojijukugo — each with subcategories (Godan by ending, Ichidan, transitive/intransitive, `i`/`na`/`no`/`taru` adjectives, …) | ✅ `sense_tags.kind='pos'`, 52 distinct codes. The deepest tree and the one we are best equipped for — the same codes already drive the grammar pills. The nesting (Verbs → Godan → "with `bu` ending") maps onto our code families (`v5b`, `v5g`, `v5k`…). |
| **Expressions**         | Expressions, Idiomatic expressions, Proverbs                                                                                                                                                                                                                   | ✅ POS `exp` + misc `id`, `proverb`, `quote`.                                                                                                                                                                                                               |
| **Subject**             | ~35 fields: Anatomical, Baseball, Botany, Buddhist, Computer terminology, Mahjong, Shogi, Sumo, …                                                                                                                                                              | ✅ `sense_tags.kind='field'`, 81 distinct codes — a SUPERSET of the target list, so this is a naming/curation question rather than a data one.                                                                                                              |
| **Slang and dialect**   | Children's, Familiar, Female/Male term, Manga slang, Polite/Honorific/Humble, and ~10 regional -ben (Kansai, Kyoto, Osaka, Tohoku, Tsugaru, Ryukyuan…)                                                                                                         | ⚠️ `kind='dialect'` (9 codes) + `kind='misc'` (`chn`, `fam`, `fem`, `male`, `m-sl`, `hon`, `hum`, `pol`, `sl`). Every regional -ben on the list has a JMdict code. **But see the dataset caveat below.**                                                    |
| **Other**               | Abbreviations, Archaisms, Colloquialisms, Obsolete terms, Kana-only                                                                                                                                                                                            | ✅ misc `abbr`, `arch`, `col`, `obs`, `uk`.                                                                                                                                                                                                                 |

**Dataset caveat, measured 2026-08-02.** Counts taken against the dev DB understate the slang/dialect vertical severely — it is built `variant: common` (`meta.variant`), and dialect words are mostly NOT common: Kansai-ben shows 26 words locally against the hundreds such a list should hold. The tags are all present and correctly populated; the entries carrying them are simply filtered out of the common subset. **Size this vertical against a `--full` build, not the dev DB**, or it will look not-worth-building when it is.

**Conclusion: no additional data source is needed.** All seven categories map onto tags we already store — the gap is presentation, not data. The one genuine judgement call is Subject, where our 81 field codes exceed the ~35 wanted and need curating down rather than filling in.
