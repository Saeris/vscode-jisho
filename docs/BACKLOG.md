...
- **Per-feature suites: deferred, and the file split is the wrong lever.** `workers: 1` + `fullyParallel: false` (forced: one debug port, one VS Code) means suites do not overlap, and each new file pays its own `beforeAll(launchVSCode)` — so splitting `visual.e2e.ts` six ways trades one boot for six, moving the suite the wrong way on the axis that actually annoys. Navigability is the real goal, and `test.describe` blocks in a serial file deliver it at zero launch cost, as the light-theme block now shows. Revisit only if a feature vertical needs genuinely different launch settings, which is the one thing a describe block cannot express.

Remaining work is the restructure itself, which is now cosmetic. Not blocking v1.
