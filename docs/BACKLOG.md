...
- **POS semantic highlighting** — PROTOTYPE SHIPPED (2026-07-18): a DocumentSemanticTokensProvider on markdown/plaintext colors Japanese by part of speech at the MORPHEME level (auxiliaries distinct from their verb stem, so conjugation structure is visible), ruby-markup aware (a {食|た}べる group colors whole as its word). POS maps onto built-in token types so every theme colors it: verb→function, noun→variable, adjective→type, adverb→property, particle→keyword, auxiliary→macro. Off by default (`vscode-jisho.highlighting.enabled`) — recoloring documents is opinionated; the toggle applies live via onDidChangeSemanticTokens. Pure-kana runs stay uncolored (same tokenizer constraint as the hover — wrong boundaries would teach wrong boundaries). Awaiting the user's visual refinement pass: the type mapping is the first knob to turn; custom token types + semanticTokenScopes fallbacks are the upgrade path if the built-ins prove too arbitrary.
  ...
  **Round 2 direction — custom palettes + color-vision accessibility (user design input, 2026-07-18).** The user authored a POS palette for their Japanese slide decks (11 categories: pronoun, noun, adnominal, adjectival noun, adjective, particle, adverb, conjunction, interjection, verb, auxiliary verb; light + dark variants; designed toward CVD friendliness) and flagged that theme-hooking cannot serve color-vision-deficient users — different palettes per CVD type may be needed. Decisions:
  ...
  - **OPEN: example sentences colour only 4 of the 9 categories** (user report, measured 2026-08-02). Example colouring rides the build-time `[word](pos:id)` links, and `LINKABLE_POS` in `build-data.ts` emits links for content words only — `n`/`v`/`adj`/`adv`. Measured over the shipped sentences: **68.7% of characters sit inside a link** and can be coloured; the other **31.3% cannot** — particles, auxiliaries, pronouns, adnominals and utterances render as plain foreground. That is why examples look unlike the editor highlighting, which colours all nine. The particles inverting the emphasis (uncoloured connectives ending up the brightest text) is the same defect seen from the other side.

    **Decided approach:** keep the build-time pass and extend the markup so unlinkable words carry a POS with an EMPTY id — `[は](p:)` — meaning "colour this, don't link it". `LINKABLE_POS` stays as the LINK policy (linking a particle to a JMdict entry is useless); it just stops doubling as the COLOUR policy. The build already calls `segment()` on every sentence and already has all nine categories — it discards five of them today, so this adds no tokenization.

    **Rejected: tokenizing in the webview.** Measured both costs rather than assuming. Storing the spans is **+13% raw on `ja_furigana` but only +1.1% compressed (~+0.05 MB on the artifact)** — `](p:)` repeated 133k times is nearly free to zstd, so raw size is the wrong metric for a compressed artifact. Against that, the tokenizer is a **4.9 MB native Node addon** that cannot load in a webview at all; it would need a WASM build of Lindera plus its IPADIC dictionary in the webview bundle. It would also re-derive, from surface text alone, matches the build makes against JMdict with lemma+reading — so the two passes could disagree, colouring a word one way while it links another.
