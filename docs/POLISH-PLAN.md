...
4. **Full webview E2E (Playwright driving Electron).** The only layer that reaches the webview DOM — drawing canvas, rendered strokes, candidate chips, navigation. **Also the visual-iteration loop**: launch → drive → screenshot → the agent refines UI against real pixels. Highest setup; foundational for the visual-polish goal, so not "overkill" here.
   ...
   **Status: harness built and proven** (`e2e/`, `playwright.config.ts`) — a full run passed 3/3 in ~15s, including a DB-backed search and a real workbench screenshot. Hard-won findings:
   ...
   - **Two different mutex messages — one is noise, one is fatal. Don't confuse them.**
     - `Error: mutex already exists` is **non-fatal noise**; it appears on runs that then succeed. Don't chase it.
     - `checkInnoSetupMutex: vscode-updating is held, waiting up to 30s for setup to finish...` is **fatal**: a VS Code _installer update_ is running on the machine, every spawned instance blocks on it, no window is created, and CDP reports **zero targets**. It surfaces only as "workbench window not found" — identical to the leaked-children symptom, which is how you end up debugging the wrong thing. It's transient: let the update finish and re-run. (Diagnosed by spawning `Code.exe` by hand and reading its stdout — the harness swallows it with `stdio: "ignore"`.)
   - **Pin the VS Code version** (`downloadAndUnzipVSCode("1.128.1")`, not `"stable"`). `"stable"` re-resolves per run, so a release silently swaps the binary (and re-downloads 224MB) under a suite that was green an hour ago. Bump deliberately.
   - **Cleanup must tree-kill.** `proc.kill()` only signals the launcher; VS Code's children survive and can hold the install/profile. We use `taskkill /T /F` on Windows and a negative-pid group signal on POSIX (hence `detached: true` there), always scoped to the PID we spawned. **Verified:** three consecutive runs pass (50s cold / 14.9s / 14.7s warm) leaving **zero** lingering processes and the debug port released. That repeatability check is the real proof — an earlier version "passed once then degraded" because children leaked.
   - **RESOLVED — the renderer-hang / "Device or resource busy" episode.** Two unrelated problems looked like one: (1) **GitKraken** held a file handle on `.vscode-test/`, blocking a clean re-download (nothing to do with this harness, and invisible to process-path queries); (2) the missing tree-kill above. With the folder cleared and tree-kill in place, the harness is green and repeatable. If launches ever hang on a blank `workbench.html` again, suspect leaked children or an external indexer holding `.vscode-test/` — not the mutex.
