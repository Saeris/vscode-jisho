...
test("capture: the browse categories", async () => {
  // The TOP level of the drill-down, paired with `browse-categories` (the level below it) in the
  // README. That pairing is why this has a scenario at all: the image existed as an orphan for a
  // while, generated by an older run and never regenerated, so it kept the 300px width from before
  // the docs sidebar was widened to 410. Side by side with its sibling the mismatch was obvious.
  const win = vscode!.window;
  await captureBothThemes(vscode!, "browse-vocab", async () => {
    ...
    await frame
      ...
      .click();
    // A category with its count, which is what distinguishes this level from the one below.
    await expect(
